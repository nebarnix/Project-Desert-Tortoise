...
function [dataStreamOut, d_freqi, d_locksigi, firstLock] = CarrierTrackPLL(dataStreamIn, Fs, freqRange, d_lock_threshold, loopbw_acq, loopbw_track, lockSigAlpha)
...
for idx=1:numel(dataStreamIn)
    ...
    if d_locksig > d_lock_threshold && firstLock == 0 %This needs to be a moving average or at least somewhat smoothed
        ...
        %d_alpha = alpha_track;
        %d_beta = d_alpha/25;
    elseif d_locksig < d_lock_threshold
        bw = loopbw_acq;        
        d_alpha = (4 * damp * bw) / (1 + 2 * damp * bw + bw * bw);
        d_beta = (4 * bw * bw) / (1 + 2 * damp * bw + bw * bw);
    end
